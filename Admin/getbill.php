...
class PDF extends FPDF
{
    ...
    function BasicTable()
    {
        ...
        if ($row = mysqli_fetch_assoc($data)) {
            $sum++;
            $this->Ln();
            $this->Cell(70, 6, 'User Reference : #' . $row['request_id']);
            $this->Ln();
            ...
            $this->Ln();
            $this->Cell(70, 6, 'Date : ' . $row['date_updated']);
            $this->Ln();
            $this->Ln();
            $this->Cell(70, 6, 'Seats : ' . $row['weight']);
            $this->Ln();
            $this->Cell(70, 6, 'Address : ' . $row['red_address']);
            $this->Ln();
            $this->Cell(70, 6, 'Phone: ' . $row['res_phone']);
            $this->Ln();
            $this->Ln();
